...
document.addEventListener("click",e=>{
 ...
 if(p){e.preventDefault();e.stopImmediatePropagation();if(typeof window.openCompose==="function")return window.openCompose();show("POST ANONYMOUSLY","<p class='muted'>No account required.</p><textarea id='bootPostText' class='field area' maxlength='500' placeholder='Share your thoughts...'></textarea><div class='counter' id='bootPostCounter'>0/500</div><button class='primary' id='bootPost'>SEND</button>");const ta=document.getElementById("bootPostText");ta.oninput=()=>document.getElementById("bootPostCounter").textContent=ta.value.length+"/500";document.getElementById("bootPost").onclick=async()=>{const text=ta.value.trim();if(!text)return show("POST ERROR","<p>Write something first.</p>");const r=await fetch(SB_URL+"/rest/v1/messages",{method:"POST",headers:{"apikey":SB_KEY,"Authorization":"Bearer "+SB_KEY,"Content-Type":"application/json","Prefer":"return=minimal"},body:JSON.stringify({content:text,author_id:"Anonymous"})});if(!r.ok){let msg="Could not post right now.";try{const x=await r.json();if(x?.message)msg=x.message}catch{}return show("POST ERROR","<p>"+String(msg).replace(/[&<>]/g,"")+"</p>")}show("POSTED","<p>Your anonymous post was sent successfully.</p>");setTimeout(hide,900)}}
},true);
