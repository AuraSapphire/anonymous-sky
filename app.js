...
async function loadReleaseMusic(){try{const r=await fetch(MUSIC_RELEASE_API,{headers:{Accept:"application/vnd.github+json"}});if(!r.ok)throw Error("No music release yet");const release=await r.json();const assets=(release.assets||[]).filter(a=>/\.(mp3|wav|ogg|m4a|aac)$/i.test(a.name));const bodyLinks=[...String(release.body||"").matchAll(/\\((https:\\/\\/github\\.com\\/user-attachments\\/files\\/[^)]+\\.(?:mp3|wav|ogg|m4a|aac))\\)/gi)].map(m=>m[1]);const tracks=assets.map(a=>({name:a.name,url:a.browser_download_url}));for(const url of bodyLinks){if(!tracks.some(t=>t.url===url))tracks.push({name:decodeURIComponent(url.split("/").pop()||"").replace(/\\.(mp3|wav|ogg|m4a|aac)$/i,"").replace(/\\.\\-/g," - ").replace(/\\./g," ").replace(/\\bSPOTISAVER\\b/gi,"").replace(/\\s{2,}/g," ").trim()+url.match(/\\.(mp3|wav|ogg|m4a|aac)$/i)?.[0]||".mp3",url})}if(!tracks.length)throw Error("No audio files in latest release");musicTracks=tracks;musicIndex=0;music.src=musicTracks[0].url;music.load();const title=document.getElementById("musicTitle");if(title)title.textContent=musicTracks[0].name;const ready=document.getElementById("readyText");if(ready)ready.textContent="Ready • "+musicTracks.length+" song"+(musicTracks.length===1?"":"s")}catch(e){console.warn("GitHub release music unavailable:",e)}}
