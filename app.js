...
const MUSIC_RELEASE_API="https://api.github.com/repos/AuraSapphire/anonymous-sky/releases/latest";
let musicTracks=[],musicIndex=0;
async function loadReleaseMusic(){try{const r=await fetch(MUSIC_RELEASE_API,{headers:{Accept:"application/vnd.github+json"}});if(!r.ok)throw Error("No music release yet");const release=await r.json();const assets=(release.assets||[]).filter(a=>/\.(mp3|wav|ogg|m4a|aac)$/i.test(a.name));const bodyLinks=[...String(release.body||"").matchAll(/\((https:\/\/github\.com\/user-attachments\/files\/[^)]+\.(?:mp3|wav|ogg|m4a|aac))\)/gi)].map(m=>m[1]);const tracks=assets.map(a=>({name:a.name.replace(/\bSPOTISAVER\b/gi,"").replace(/\s{2,}/g," ").trim(),url:a.browser_download_url}));for(const url of bodyLinks){if(!tracks.some(t=>t.url===url)){let name=decodeURIComponent(url.split("/").pop()||"").replace(/\.(mp3|wav|ogg|m4a|aac)$/i,"").replace(/\.-/g," - ").replace(/\./g," ").replace(/\bSPOTISAVER\b/gi,"").replace(/\s{2,}/g," ").trim();tracks.push({name:name+(url.match(/\.(mp3|wav|ogg|m4a|aac)$/i)?.[0]||".mp3"),url})}}if(!tracks.length)throw Error("No audio files in latest release");musicTracks=tracks;musicIndex=0;music.src=musicTracks[0].url;music.load();const title=document.getElementById("musicTitle");if(title)title.textContent=musicTracks[0].name;const ready=document.getElementById("readyText");if(ready)ready.textContent="Ready • "+musicTracks.length+" song"+(musicTracks.length===1?"":"s")}catch(e){console.warn("GitHub release music unavailable:",e)}}
function setTrack(i,autoplay=false){if(!musicTracks.length)return;musicIndex=(i+musicTracks.length)%musicTracks.length;music.src=musicTracks[musicIndex].url;music.load();const title=document.getElementById("musicTitle");if(title)title.textContent=musicTracks[musicIndex].name;if(autoplay)music.play().then(()=>setPlaying(true)).catch(()=>setPlaying(false))}
async function toggleMusic(){if(music.paused){try{await music.play();setPlaying(true)}catch{alert("No playable music release is available yet. Upload MP3s to a GitHub Release for Anonymous Sky.")}}else{music.pause();setPlaying(false)}}playBtn.onclick=toggleMusic;mainPlay.onclick=toggleMusic;
document.getElementById("mainStop").onclick=()=>{music.pause();music.currentTime=0;setPlaying(false)};document.getElementById("prevBtn").onclick=()=>setTrack(musicIndex-1);document.getElementById("mainPrev").onclick=()=>setTrack(musicIndex-1);document.getElementById("nextBtn").onclick=()=>setTrack(musicIndex+1,true);document.getElementById("mainNext").onclick=()=>setTrack(musicIndex+1,true);
...
setupDiscovery();load();setInterval(load,30000);music.volume=.7;loadReleaseMusic();
