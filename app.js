...
function render(rows){if(!rows.length){grid.innerHTML='<div class="empty">The sky is quiet. Be the first to release a thought.</div>';return}grid.innerHTML=rows.map(m=>'<article class="card"><b>Anonymous</b><small>'+ago(m.created_at)+'</small><p>'+esc(m.content)+'</p><small>♡　0　　▢　0</small></article>').join("")}
async function load(){try{const r=await fetch(rest+"?select=id,content,created_at&order=created_at.desc&limit=100",{headers});if(!r.ok)throw Error(await r.text());render(await r.json())}catch(e){console.error(e);grid.innerHTML='<div class="empty">Could not connect to the sky right now.</div>'}}
